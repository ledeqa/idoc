...
(function(){
	...
	var TEMPLATE = {
		'REQUEST_BEGIN'					: '<table class="table table-bordered" id="core_edit_table"><thead><tr><td class="head-expander"></td><th class="head-name">参数名称</th><th class="head-desc">含义</th><th class="head-type">类型</th><th class="head-required">是否必须</th><th class="head-remark">备注</th><th class="head-mock">mock</th></tr></thead><tbody>',
		'REQUEST_BEGIN_EDIT'			: '<table class="table table-bordered" id="core_edit_table"><thead><tr><td class="head-expander"></td><th class="head-op">OP</th><th class="head-name">参数名称</th><th class="head-desc">含义</th><th class="head-type">类型</th><th class="head-required">是否必须</th><th class="head-remark">备注</th><th class="head-mock">mock</th></tr></thead><tbody>',
	...
	param.getTableId = function(){
		return "core_edit_table";
	}
	param.getAddParamBtnId = function(){
		return "core_add_param";
	}
	
	param.init = function(dictParamObj, editMode){
        		...
        		success: function(json){
        			if(json.retCode == 200){
        				if(json.dictList){
        					...
        					var options = '';
        					var checkDictId = $('#editDictId').val().trim();
        					...
        					bootbox.dialog({
        		        		...
        		        		buttons:{
        		        			createDict:{
        		        				...
        		        				callback: function(){
        		        					window.open("editDict.html?productId="+dict.productId);
        		        				}
        		        			},
        		        			ok:{
        		        				label: '确定',
...
//        		    						var optionObj = new Option(selectedDict.dictName, selectedDict.dictId, false, true);
    ...
    function getPTDHtml(id, value, type, level) {
    	if(!value){
    		return '<td id="td-param-' + type + '-' + id + '" class="td-param ' + type + '" onclick="param.edit(' + id + ', \'' + type + '\'' + ');" ></td>';
    	}
