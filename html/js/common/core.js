...
(function(){
	...
	var TEMPLATE = {
		'REQUEST_BEGIN_EDIT'			: '<table class="table table-bordered"><thead><tr><td class="head-expander"></td><th class="head-op">OP</th><th class="head-name">参数名称</th><th class="head-desc">含义</th><th class="head-type">类型</th><th class="head-required">是否必须</th><th class="head-remark">备注</th><th class="head-mock">mock</th></tr></thead><tbody>',
		...
		'RESPONSE_BEGIN_EDIT'			: '<table class="table table-bordered"><thead><tr><th class="head-expander"></th><th class="head-op">OP</th><th class="head-name">参数名称</th><th class="head-desc">含义</th><th class="head-type">类型</th><th class="head-required">是否必须</th><th class="head-remark">备注</th><th class="head-mock">mock</th></tr></thead><tbody>',
	...
	param.init = function(reqParamObj, retParamObj){
		_productId = $('#productId').val().trim();
		
		_reqParam = reqParamObj;
		generateUniqueIdForParams(_reqParam);
		_retParam = retParamObj;
		generateUniqueIdForParams(_retParam);
        		        		...
        		        		buttons:{
        		        			ok:{
        		    					...
        		    					callback: function() {
        		    						...
        		    						data.dictId = dictId;
        		    						$.ajax({
        		    							url: '/dict/queryDict.html',
        		    							type: 'POST',
        		    							dataType: 'json',
        		    							data: data,
        		    							success: function(json){
        		    								if(json.retCode == 200){
        		    									var dict = json.retContent;
...
//        		    						var optionObj = new Option(selectedDict.dictName, selectedDict.dictId, false, true);
    ...
    function getRequiredEditSelectHtml(id, isRequired) {
        ...
        str += '<td id="td-param-required-' + id + '" class="td-param">' +
        		'<select class="form-control" id="select-required-' + id + '" onchange="">';
        for (var i = 0; i < typeListNum; i++) {
            var item = typeList[i];
