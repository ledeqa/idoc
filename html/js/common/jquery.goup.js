...
(function ( $ ) {
	...
	$.goup = function(user_params) {
		...
		$('body').append('<div style="display:none" class="down-container"></div>');
		var downContainer = $('.down-container');
		$(downContainer).html('<div class="down-arrow"></div>');
		...
		$(downContainer).css(containerStyle);
		$(downContainer).css({'bottom': params.bottomOffset - params.containerSize});
		...
		$(downArrow).css(arrowStyle);
		$(downArrow).css({'margin': '15px auto', 
		...
		if (!params.alwaysVisible) {
			$(window).scroll(function(){
				if ($(window).scrollTop() >= params.trigger && !isHidden) {
					...
					do_animation($(downContainer), 'show', params.entryAnimation);
                    if (textContainer)
				...
				if ($(window).scrollTop() < params.trigger && !isHidden) {
					do_animation($(container), 'hide', params.entryAnimation);
					do_animation($(downContainer), 'hide', params.entryAnimation);
                    if (textContainer)
                        do_animation($(textContainer), 'hide', params.entryAnimation);
				}
			});
		...
		$(downContainer).on('click', function(){
			if (notClicked) {
				notClicked = false;
				$('html,body').animate({ scrollTop: $('.footer').offset().top }, params.goupSpeed, function(){ notClicked = true });
			}
			return false;
