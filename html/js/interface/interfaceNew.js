 ...
 * 接口管理页面js
 */
//var oldInterName="五指山";
//var interNameExist=0;
//var isReEditing=0;
var isEditing=0;
var moduleList = '';
...
//		if(interNameExist==1){
//			showAlertMessage('接口名称已存在，请重新输入');
//			return;
//		}
...
//		isReEditing=1;
		isEditing=1;
//		oldInterName=$('#interName').val().trim();
...
//	$('#interName').change(function(){
//	    var status=$('#interName').attr("readonly");
//	    if(status=="readonly"){
//		   return;
//	     }
////	    if(isReEditing==1&&oldInterName==$('#interName').val().trim()){
////	    	$('#labelInterName').empty();
//////	    	interNameExist=0;
////	    	return;
////	    }
//	    var interName=$('#interName').val().trim();
//	    var data={};
//	    data.interName=interName;
//	    $.ajax({
//		    url: '/idoc/inter/checkInterName.html',
//		    type: 'POST',
//		    dataType: 'json',
//		    data: data,
//		    success: function(json){
//			if(json.retCode==400){
//				$('#labelInterName').text("<br>");
//				$('#labelInterName').text("接口名称已存在，请重新输入");
//				interNameExist=1;
//			    $('#interName').focus();
//			}else{
//				interNameExist=0;
	...
	if(!isNullOrEmpty(interfaceStatus)){
		...
		case 10:  // 暂存中
			...
			if(role == "管理员" || role == "后台开发负责人" || (role == "后台开发")){
				...
				if($('#isNeedInterfaceTest').val().trim()!=0){					
				    html += '<button class="btn btn-warning btn-sm" id="submitToTest">提交测试</button>';
				}else{
...
function convertStatus(frontStatus){
	if(frontStatus==2||frontStatus==301||frontStatus==310||frontStatus==3){
		frontStatus=1;
	}
	if(frontStatus==4){
		frontStatus=5;
	}
	if(frontStatus==7||frontStatus==8){
		frontStatus=6;
	}
	return frontStatus;
}
	...
	$.ajax({
		...
		success: function(json){
			if(json.retCode == 200){
				...
				var frontStatus=inter.interfaceStatus;          //gaidong!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				var status = convertStatus(frontStatus);
				var statusStr = getStatus(status);
				$('#lb_interStatus').text(statusStr);
				$('#lb_interStatus').css('color','red');
				$('#lb_interStatus').css('font-size',16);
			...
			for(var j=0; j<pageLen; j++){
				...
				if(page_id==pageId){
					...
					for(var m=0; m<interLen; m++){
						var interface = interList[m];
						inters += "<option value=\""+interface.interfaceId+"\">" + interface.interfaceName + "</>";
					}
