...
function getCurrentInter(){
	...
	$.ajax({
		url: '/idoc/inter/searchInterface.html',
		type: 'GET',
		dataType: 'json',
		data: data,
...
//	}
	...
	$.ajax({
		url: '/idoc/inter/searchInterfaceVersion.html',
		type: 'GET',
		dataType: 'json',
		data: data,
		success: function(json){
			if(json.retCode == 200){
				...
				var operator  = json.operator;    //从后端取出的操作人姓名
				...
				if(inter != null){
					displayInterVersion(inter,operator);
				}
			}else{
...
// 显示选中的历史版本信息
function displayInterVersion(inter,operator){
	...
	if(!isNullOrEmpty(inter.interfaceId)){
		$('#interfaceHistoryVersion').html('<button class="btn btn-primary btn-xs" onclick="rollbackInterfaceHistory();">回滚到此版本</button>');
	}
	//控制ftl模板显示还是隐藏
...
function rollbackInterfaceHistory(){
	var data = {};
	data.interfaceForm = newInterfaceForm(_interObj);
	$.ajax({
		url: '/idoc/inter/rollbackInterfaceHistory.html',
		type: 'POST',
		dataType: 'json',
		async: false,
		...
		success: function(json){
			if(json.retCode == 200){
				...
				showAlertMessage('回滚历史版本成功，请刷新原接口页面查看。');
			}else{
				showAlertMessage('回滚失败。');
			}
		}
	});
}
...
function newInterfaceForm(interObj){
	...
	interfaceForm.creatorId = interObj.creatorId;
	...
	interfaceForm.interfaceStatus = interObj.interfaceStatus;
