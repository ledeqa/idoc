package com.idoc.protocol.test.tcp;
...
class ClientHandler extends IoHandlerAdapter {
    ...
    public void sessionOpened(final IoSession session) throws Exception {
        ...
        session.write(req); //写数据，该操作是异步的
        new Thread(new Runnable(){
        	public void run(){
        		RequestMessage requst = new RequestMessage();
        		for(int i = 0; i < 100; i++){
        			requst.setRequest("TCP-hello-1-" + i);
                    session.write(requst); 
                    try{
                    	Thread.sleep(100);
        ...
        new Thread(new Runnable(){
        	public void run(){
        		RequestMessage requst = new RequestMessage();
        		for(int i = 0; i < 100; i++){
        			requst.setRequest("TCP-hello-2-" + i);
                    session.write(requst); 
                    try{
                    	Thread.sleep(100);
                    }catch(InterruptedException e){}
    ...
    public void messageReceived(IoSession session, Object message){
    	if(message instanceof ResponseMessage){
	    	...
	    	System.out.println("TCP client接受到 [" + session.getRemoteAddress() + "] 的信息为：" + res.getResponse() + "-" + i);
	    	i++;
    	}
    }
    /**
