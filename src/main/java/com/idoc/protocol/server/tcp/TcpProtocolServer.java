...
import com.idoc.protocol.message.codec.MessageCodecFactory;
import com.idoc.protocol.server.ProtocolServer;
import com.idoc.protocol.util.ContextConstant;
import com.idoc.protocol.util.LoadProtocolConfig;
import com.netease.common.util.StringUtil;

@Service("tcpProtocolServer")
public class TcpProtocolServer implements ProtocolServer {
	...
	public void start() {
		...
		try {
			port = LoadProtocolConfig.getProtocolConfig(ContextConstant.PROTOCOL_TCP_PORT);
			charsetName = LoadProtocolConfig.getProtocolConfig(ContextConstant.PROTOCOL_TCP_CHARSET);
		} catch (IOException e) {
			LogConstant.debugLog.info("加载protocol.properties文件时异常：", e);
		}
		if (StringUtil.isEmpty(port)) {
			LogConstant.runLog.info("protocol.properties文件中没有关于TCP端口port的配置！");
			LogConstant.runLog.info("TcpProtocolServer服务启动失败！");
			return;
		} else {
			try{
				...
				LogConstant.debugLog.info("TcpProtocolServer服务端口转换异常：" + e);
				return;
			}
		}

		LogConstant.runLog.info("===========================TcpProtocolServer服务开始启动===========================");
		...
		// 读写通道10秒内无操作进入空闲状态
		// cfg.setIdleTime(IdleStatus.BOTH_IDLE, 4000);
		...
		try {
			...
			LogConstant.debugLog.info("TcpProtocolServer服务绑定端口时异常：", e);
			return;
		}

		LogConstant.runLog.info("服务监听的端口为：" + port + "   编码方式为：" + charsetName);
		LogConstant.runLog.info("===========================TcpProtocolServer服务启动成功！===========================");
	}
	...
	public void stop() {
		if (null != acceptor) {
			...
			acceptor.dispose();
			acceptor = null;
			LogConstant.runLog.info("TcpProtocolServer服务关闭成功！");
		}
	}
